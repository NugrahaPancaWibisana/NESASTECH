<x-layout>
    ...
    <section class="pl-[230px] w-full h-full p-5">
        <div class="flex flex-col w-full h-full gap-5 p-5 bg-white border border-gray-300 rounded-xl">
            ...
            <form action="{{ route('keuangan.update', ['keuangan' => $keuangan->id]) }}" method="POST"
                ...
                <div class="w-full">
                    <p>
                        ...
                        <br />
                        <br />
                        <strong>Uang sekarang - jumlah total uang = pemasukan hari ini</strong>
                    </p>
                </div>
                        ...
                        <div class="w-full px-2 bg-white border border-black rounded-xl">
                            ...
                            <div class="flex items-center justify-center gap-3">
                                ...
                                <input class="w-full h-10 border-none outline-none" type="number" id="masuk"
                                    name="masuk" placeholder="Pemasukan" value="{{ $keuangan->masuk }}">
                            </div>
                        </div>
                        @error('masuk')
                            <p>{{ $message }}</p>
                        ...
                        <div class="w-full px-2 bg-white border border-black rounded-xl">
                            ...
                            <div class="flex items-center justify-center gap-3">
                                ...
                                <input class="w-full h-10 border-none outline-none" type="number" id="keluar"
                                    name="keluar" placeholder="Pengeluaran" value="{{ $keuangan->keluar }}">
                            </div>
                        </div>
                        @error('keluar')
                            <p>{{ $message }}</p>
                ...
                <div class="w-full p-1 px-2 bg-white border border-black rounded-xl">
                    ...
                    <textarea placeholder="Keterangan" class="w-full border-none outline-none resize-none h-44" id="keterangan"
                        name="keterangan">{{ $keuangan->keterangan }}</textarea>
                    @error('keterangan')
                        <p>{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end w-full gap-3">
                    <a href="/"
                        class="flex items-center justify-center h-10 px-10 border border-black outline-none btn rounded-xl"
